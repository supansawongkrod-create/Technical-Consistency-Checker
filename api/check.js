...
async function blobToGemini(meta, key) {
  // Generate a short-lived signed GET URL for this exact private blob.
  // This avoids depending on SDK get() behavior and uses the same
  // signed-URL mechanism that already succeeds for uploads.
  ...
  const blobResp = await fetch(presignedUrl, { method: 'GET' });
  if (!blobResp.ok || !blobResp.body) {
    const t = await blobResp.text().catch(() => '');
    throw new Error(
      `Could not read uploaded file: ${meta.name}` +
      (t ? ` (${t.slice(0,120)})` : ` (${blobResp.status})`)
    );
  }
  ...
  const contentLength = Number(blobResp.headers.get('content-length'));
  ...
  if (!uploadUrl) {
    throw new Error(`Gemini did not return an upload URL for ${meta.name}`);
  }
  ...
  if (!up.ok || !info?.file?.uri) {
    throw new Error(
      `Gemini upload failed for ${meta.name}: ${info?.error?.message || up.status}`
    );
  }
